 ...
 *
 * Two authored fields were deliberately left behind for four phases. The note
 * here said `passiveTrait` and `activeSkillArchetype` "describe verbs no phase
 * has built yet", and that carrying data nothing reads is how the previous
 * config reached four thousand lines. That was right then and stopped being
 * right: `getHeroPassiveMultipliers` reads the trait and
 * `getUniqueRelicMultipliers` reads the archetype, both are pinned against
 * fixtures, and **both were uncallable from a real roster** because the
 * catalogue did not carry what they take. Two of the fourteen damage
 * multipliers were unreachable for want of two strings a hero.
 *
 * They are generated from `__fixtures__/unique-relics.json` rather than typed —
 * it already records every hero's pair from the shipped source — and
 * `heroes.test.ts` checks all sixty-five back against it.
 */
