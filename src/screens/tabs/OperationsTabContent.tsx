...
import {
  ...
  EXPEDITION_RARITY_META,
  formatDurationShort,
} from '../gameScreenShared';
import type { ExpeditionType, ExpeditionRarity } from '../gameScreenShared';
  ...
  ({
    ...
    const dungeonLanes = [
      ...
      },
      {
        ...
        status:
          !state.highestWaveReached || state.highestWaveReached < 80
      ...
      <>
        {tab === 'operations' && (
          <View style={styles.operationsTab}>
            ...
            {renderSubTabBar(
              (['miniops', 'dungeonops', 'facilities', 'expeditions'] as const).map(st => ({
                id: st,
                label:
                  st === 'miniops'
                    ? 'Mini Ops'
            ...
            {operationsSubTab === 'miniops' && (
              <View style={styles.facilitiesSection}>
                ...
                <Text style={styles.facilitiesDesc}>
                  Tactical actions on a 4-hour cooldown from when each one is used.
                </Text>
                <View style={styles.facilityCard}>
                  <Text style={styles.facilityName}>Dice Protocol</Text>
                  <Text style={styles.facilityBonusText}>
                    Status: {canPlayDiceToday ? 'Ready' : `${formatDurationShort(diceRemainingMs)} remaining`}
                  </Text>
                ...
                <View style={styles.facilityCard}>
                  ...
                  <Text style={styles.facilityBonusText}>
                    Reveal 1 of 3 intel outcomes (gold, shards, or combat telemetry).
                  </Text>
                  <Text style={styles.facilityBonusText}>
                    Status: {canPlayReconToday ? 'Ready' : `${formatDurationShort(reconRemainingMs)} remaining`}
                  </Text>
                  <Pressable
                ...
                <View style={styles.facilityCard}>
                  ...
                  <Text style={styles.facilityBonusText}>
                    Crack a cache for diamonds, or salvage partial gold on a jam.
                  </Text>
                  <Text style={styles.facilityBonusText}>
                    Status: {canPlayLockpickToday ? 'Ready' : `${formatDurationShort(lockpickRemainingMs)} remaining`}
                  </Text>
                  <Pressable
                ...
                <View style={styles.facilityCard}>
                  ...
                  <Text style={styles.facilityBonusText}>
                    Status: {canPlayTargetToday ? 'Ready' : `${formatDurationShort(targetRemainingMs)} remaining`}
                  </Text>
                  <Pressable
                ...
                <View style={styles.facilityCard}>
                  ...
                  <Text style={styles.facilityBonusText}>
                    Pick one contract for this cycle and claim when objective is done.
                  </Text>
                  {state.miniBounty ? (
                    <>
                      <Text style={styles.facilityBonusText}>Active: {state.miniBounty.title}</Text>
                      <Text style={styles.facilityNextBonus}>
                        Progress: {Math.min(activeMiniBountyProgress, state.miniBounty.targetValue)}/
                        {state.miniBounty.targetValue}
                      </Text>
                      <Text style={styles.facilityNextBonus}>
                        Reward: +{fmt(state.miniBounty.rewardGold)} gold, +{fmt(state.miniBounty.rewardShards)} shards,
                        +{state.miniBounty.rewardDiamonds} diamonds
                      </Text>
                      <Pressable
                      ...
                      >
                        <Text style={styles.warPanelActionText}>
                          {canClaimMiniBounty ? 'Claim Bounty' : 'In Progress'}
                        </Text>
                      </Pressable>
                    </>
                  ) : (
                    <>
                      {!canStartBountyToday && (
                        <Text style={styles.facilityNextBonus}>
                          Next draft in: {formatDurationShort(bountyRemainingMs)}
                        </Text>
                      )}
                ...
                {dungeonLanes.map(lane => (
                  <View key={lane.id} style={styles.facilityCard}>
                    ...
                    {lane.id === 'rift' && (
                      <>
                        <Text style={styles.facilityNextBonus}>
                          Dungeon Level: {riftDungeonLevel} (Boss Lv {riftDungeonLevel * 10})
                        </Text>
                        <Text style={styles.facilityNextBonus}>
                          Free entries used today: {riftEntriesUsed}/{riftEntryCap}
                        </Text>
                        <Text style={styles.facilityNextBonus}>Raid Tickets: {riftRaidTickets}</Text>
                        <Text style={styles.facilityNextBonus}>
                          Last run damage: {Math.round(lastRiftBossDamagePct * 100)}%
                        </Text>
                      </>
                    )}
                    {lane.id === 'gold' && state.highestWaveReached >= 80 && (
                      <>
                        <Text style={styles.facilityNextBonus}>
                          Vault Level: {treasureDungeonLevel} ({treasureDungeonLevel * 2 + 3} waves)
                        </Text>
                        <Text style={styles.facilityNextBonus}>
                          Free entries used today: {treasureEntriesUsed}/{treasuryEntryCap}
                        </Text>
                        <Text style={styles.facilityNextBonus}>Raid Tickets: {riftRaidTickets}</Text>
                        <Text style={styles.facilityNextBonus}>
                          Last haul: {Math.round(lastTreasureHaulPct * 100)}%{lastTreasureWiped ? ' (wiped)' : ''}
                        </Text>
                      </>
                    )}
                    {lane.id === 'rift' || lane.id === 'gold' ? (
                      <View style={styles.warPanelActionRow}>
                        <Pressable
                          style={[
                            ...
                            !(lane.id === 'gold' ? canRunTreasuryEntry : canRunRiftEntry) &&
                              styles.warPanelActionBtnDisabled,
                          ]}
                        ...
                        <Pressable
                          style={[
                            styles.warPanelActionBtn,
                            !(lane.id === 'gold' ? canRaidTreasury : canRaidRift) && styles.warPanelActionBtnDisabled,
                          ]}
                      ...
                      <Pressable
                        style={[styles.warPanelActionBtn, !lane.actionable && styles.warPanelActionBtnDisabled]}
                      ...
                      return (
                        <View key={exp.id} style={styles.expeditionQueueCard}>
                          <Text style={styles.expeditionQueueName}>
                            ...
                            {exp.type === 'ruins' && '🏛️'}
                            {exp.type === 'vault' && '🔐'}
                            {exp.type === 'abyss' && '🌑'} {exp.type.charAt(0).toUpperCase() + exp.type.slice(1)} •{' '}
                            {exp.rarity}
                          </Text>
                          ...
                          {isComplete && (
                            <Pressable style={styles.expeditionClaimBtn} onPress={() => completeExpedition(exp.id)}>
                    ...
                    return (
                      <>
                        <View style={styles.expeditionStartHeaderRow}>
                          ...
                          <Pressable
                            style={[
                              styles.expeditionRefreshBtn,
                              !canAffordRefresh && styles.expeditionRefreshBtnDisabled,
                            ]}
                            disabled={!canAffordRefresh}
                            onPress={refreshExpeditionContracts}
                          >
                            <Text style={styles.expeditionRefreshBtnText}>
                              Refresh ({fmt(EXPEDITION_CONTRACT_REFRESH_GOLD_COST)} 💰)
                            </Text>
                          </Pressable>
                        </View>
                        ...
                        {launchableTypes.length === 0 && (
                          <Text style={styles.expeditionNoLaunchText}>
                            All contracts are currently active. Claim one to launch a new run.
                          </Text>
                        )}
                          ...
                          return (
                            ...
                            >
                              <View style={styles.expeditionStartCardLeft}>
                                <Text style={styles.expeditionStartCardName}>
                                  {cfg.icon} {cfg.name}
                                </Text>
                                <Text style={styles.expeditionStartCardMeta}>
                                  {cfg.rarity} • {formatDurationShort(cfg.durationMs)}
                                </Text>
                              ...
                              <View style={styles.expeditionStartCardRight}>
                                <Text
                                  style={[
                                    styles.expeditionStartCardCost,
                                    !canStartNow && styles.expeditionStartCardCostDisabled,
                                  ]}
                                >
                                  {fmt(cfg.goldCost)} 💰
                                </Text>
                                <Text
                                  style={[
                                    styles.expeditionStartCardStatus,
                                    !canStartNow && styles.expeditionStartCardStatusDisabled,
                                  ]}
                                >
